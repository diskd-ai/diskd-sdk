...
test('operatives.list decodes intelAccess to fileAccess', async () => {
    ...
    await withFetchMock(() => new Response(JSON.stringify([wireOperative]), {
        status: 200,
...
test('operatives.getBySlug sends projectId and slug query params', async () => {
    ...
    await withFetchMock(() => new Response(JSON.stringify(wireOperative), {
        ...
        assert.equal(calls[0]?.url, 'http://app-service:3000/api/operatives/by-slug?projectId=proj-1&slug=research-agent');
    });
});
...
test('operatives.delete sends DELETE', async () => {
    const url = 'http://app-service:3000';
    await withFetchMock(() => new Response(null, { status: 204 }), async (calls) => {
        const client = diskd.platform.operatives({ auth: makeAuth(), url });
...
test('operatives.files.list decodes sourceId to path', async () => {
    const url = 'http://app-service:3000';
    await withFetchMock(() => new Response(JSON.stringify([wireFile]), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
    }), async (calls) => {
        const client = diskd.platform.operatives({ auth: makeAuth(), url });
...
test('operatives.skills.list filters to skill type only', async () => {
    const url = 'http://app-service:3000';
    await withFetchMock(() => new Response(JSON.stringify({
        registryStatus: 'ok',
...
test('operatives.skills.add sends equipmentType: skill on the wire', async () => {
    const url = 'http://app-service:3000';
    await withFetchMock(() => new Response(JSON.stringify(wireSkill), {
        status: 201,
        headers: { 'Content-Type': 'application/json' },
    }), async (calls) => {
        const client = diskd.platform.operatives({ auth: makeAuth(), url });
...
test('operatives.tools.add sends equipmentType: mcp_tool on the wire', async () => {
    const url = 'http://app-service:3000';
    await withFetchMock(() => new Response(JSON.stringify(wireTool), {
        status: 201,
        headers: { 'Content-Type': 'application/json' },
    }), async (calls) => {
        const client = diskd.platform.operatives({ auth: makeAuth(), url });
...
test('operatives client throws on HTTP error with parsed message', async () => {
    ...
    await withFetchMock(() => new Response(JSON.stringify({ message: 'Not Found' }), {
        ...
        const client = diskd.platform.operatives({ auth: makeAuth(), url });
        await assert.rejects(() => client.get('nonexistent'), (err) => {
