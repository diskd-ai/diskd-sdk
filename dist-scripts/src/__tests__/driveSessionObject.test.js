...
const makeMessage = (id, role, content) => buildMessage({ id, role, content });
const makeSessionDocument = (overrides = {}) => ({
    id: 'sess-1',
    workspaceId: 'ws-1',
    projectId: 'proj-1',
    title: 'Test Session',
...
test('session.rollback sends deleteMessages with rollbackAfterMessageId and trims local state', async () => {
    ...
    const deleteArgs = deleteCall.args;
