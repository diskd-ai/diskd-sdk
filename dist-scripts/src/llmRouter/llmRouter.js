...
const raw = (value) => {
    if (!isObject(value))
        throw new Error('Invalid LLM Router response: expected object');
    return value;
};
// ---------------------------------------------------------------------------
// Decoders
// ---------------------------------------------------------------------------
const decodeToolCall = (o) => {
    const r = raw(o);
    const fn = r.function;
    return {
        index: num(r, 'index'),
...
const decodeCompletionResult = (result) => {
    const r = raw(result);
    return {
        id: strRequired(r, 'id'),
...
const decodeEmbeddingUsage = (o) => {
    if (!isObject(o))
        return { promptTokens: 0, totalTokens: 0 };
    return {
        promptTokens: num(o, 'prompt_tokens') ?? 0,
        totalTokens: num(o, 'total_tokens') ?? 0,
    };
};
const decodeEmbeddingResult = (result) => {
    const r = raw(result);
    return {
        model: strRequired(r, 'model'),
        ...
        data: arr(r, 'data').map((item) => {
            ...
            return {
                ...
                embedding: Array.isArray(embedding)
                    ? embedding.filter((v) => typeof v === 'number')
                    : [],
            };
        }),
    };
};
const decodeModelInfo = (o) => {
    const r = raw(o);
    return {
...
const decodeOcrPage = (o) => {
    ...
    return {
        ...
        hyperlinks: arr(r, 'hyperlinks').filter((v) => typeof v === 'string'),
        header: str(r, 'header'),
        footer: str(r, 'footer'),
...
const decodeTranscribeResult = (result) => {
    const r = raw(result);
    return {
...
const encodeMessageContent = (content) => {
...
const encodeCompletionParams = (params) => ({
    ...
    ...optional('stop', params.stop !== undefined ? params.stop : undefined),
    ...optional('tools', params.tools),
...
const encodeOcrParams = (params) => ({
    model: params.model,
    document: encodeOcrDocument(params.document),
    ...optional('pages', params.pages !== null ? params.pages : undefined),
...
async function* readNdjsonStream(response) {
    if (!response.body)
        throw new Error('LLM Router stream: response has no body');
...
export const createLlmRouterClient = (params) => {
    ...
    return {
        completions: {
            ...
            stream: async function* (completionParams) {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(`LLM Router stream failed (HTTP ${response.status}): ${text.slice(0, 200)}`);
                }
        ...
        models: {
            list: async (listParams) => {
                ...
                return {
                    provider: strRequired(r, 'provider'),
                    models: arr(r, 'models').filter((v) => typeof v === 'string'),
                };
            },
            listAll: async () => {
        ...
        audio: {
            transcribe: async (transcribeParams) => {
                const authHeaders = await getAuthHeaders();
                // Build query string from optional scalar params
                const qs = new URLSearchParams();
                qs.set('model', transcribeParams.model ?? 'whisper-large-v3-turbo');
                if (transcribeParams.language !== undefined)
                    qs.set('language', transcribeParams.language);
                if (transcribeParams.prompt !== undefined)
                    qs.set('prompt', transcribeParams.prompt);
                ...
                const audioBuffer = transcribeParams.audio instanceof Uint8Array
                    ? transcribeParams.audio.buffer.slice(transcribeParams.audio.byteOffset, transcribeParams.audio.byteOffset + transcribeParams.audio.byteLength)
                    : transcribeParams.audio;
                ...
                if (!response.ok) {
                    const text = await response.text();
                    throw new Error(`LLM Router audio/transcribe failed (HTTP ${response.status}): ${text.slice(0, 200)}`);
                }
                const json = await response.json();
