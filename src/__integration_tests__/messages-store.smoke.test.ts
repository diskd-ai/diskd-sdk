...
test('integration: messagesStore.createMailbox -> listMailboxes -> init -> delete', {
  skip: skipReason,
}, async () => {
...
test('integration: mailbox.upsertFolder -> listFolders -> folder.get -> folder.delete', {
  skip: skipReason,
}, async () => {
  const store = makeStore();
  const mailboxId = uniqueMailboxId();
...
test('integration: folder.upsertBatch -> listMessages -> getMessage -> deleteBatch', {
  skip: skipReason,
}, async () => {
  const store = makeStore();
  const mailboxId = uniqueMailboxId();
