...
try {
  ...
  if (research) {
    ...
    ok(
      'agentHub.findResearchAgent',
      `no research agent found, using first: ${agents[0].id} "${agents[0].displayName}"`
    );
  }
} catch (err) {
...
if (researchAgentId) {
  try {
    ...
    const handler = new StreamProtocolHandler()
      .on('response.output_text.delta', (e: TextOutputDeltaEvent) => {
        responseText += e.delta;
      })
      .on('response.completed', (e: ResponseCompletedEvent) => {
        if (e.response.usage) {
          const u = e.response.usage;
          tokenInfo = `${u.input_tokens}in/${u.output_tokens}out`;
        }
      })
      .on('response.failed', (e: ResponseFailedEvent) => {
        throw new Error(e.response.error.message);
      })
      .on('error', (e: StreamProtocolErrorEvent) => {
    ...
    ok(
      'agentHub.invoke',
      `agent=${researchAgentId}, ${tokenInfo}, reply="${responseText.slice(0, 80)}"`
    );
  } catch (err) {
